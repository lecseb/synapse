...
Ast::Ast(const FileDescriptor *desc)
  ...
  std::string package = desc->package();
  std::replace_if(package.begin(), package.end(), ispunct, '_');

  /* add all structure definition */
  for (int32_t i = 0; i < desc->message_type_count(); i++)
    _structs.push_back(new Struct(package, desc->message_type(i)));

  for (int32_t i = 0; i < desc->enum_type_count(); i++)
    _enums.push_back(new Enum(package, desc->enum_type(i)));

  /* add contextual include */
  std::list<Struct *>::const_iterator it = get_structs_begin();
  for (; it != get_structs_end(); it++) {
    std::map<uint32_t, Field *>::const_iterator field =
      (*it)->get_fields_begin();
    for (; field != (*it)->get_fields_end(); field++)
      if (field->second->get_type() == FieldDescriptor::TYPE_BYTES)
	_includes.push_back(new Include("synapse/synapse-type.hh"));
  }
